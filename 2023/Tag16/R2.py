...
def drawWayBeam(beam,beams,field):
    curr=beam[0]
    direction=beam[1]
    while True:
...
def getScore(beam):
    ...
    while len(beams)!=0:
        field, beams = drawWayBeam(beams[0],beams, field)
        beams.pop(0)
    res=0    
    for i in field:
        for j in i:
...
for i in range(len(content)):
    res = max(res, getScore([[i,0],[0,1]]))
    res = max(res, getScore([[i,len(content[0])-1],[0,-1]]))
for i in range(len(content)):
    res = max(res, getScore([[0,i],[1,0]]))
    res = max(res, getScore([[i,len(content)-1],[-1,0]]))
